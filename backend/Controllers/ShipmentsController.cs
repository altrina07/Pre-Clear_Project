...
namespace PreClear.Api.Controllers
{
    ...
    public class ShipmentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByUser()
        {
            ...
            try
            {
                // Return list DTO with all fields needed by dashboard (title, origin/destination, value, currency, AI score, status, createdAt, referenceId)
                var list = await _service.GetUserListAsync(currentUserId.Value);
                _logger.LogInformation("GetByUser: Retrieved {Count} shipments for authenticated user {UserId}", list.Count, currentUserId);
                return Ok(list);
            }
            catch (Exception ex)
            {
